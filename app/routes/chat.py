...
def chat():
    ...
    Accepts:
        ...
        - Selected/target language via form field or JSON field 'language'
    ...
    if not language and request.is_json:
        language = request.json.get('language', 'auto') if request.json else 'auto'
    ...
    if 'audio' in request.files:
        ...
        if audio_file.filename and _allowed_audio_file(audio_file.filename):
            try:
                ...
                # Transcribe audio to text (pass the target language to Whisper if specified)
    ...
    if 'image' in request.files:
        ...
        if image_file.filename and _allowed_image_file(image_file.filename):
            try:
                ...
                # Determine target language and code
                LANG_MAP = {
                    'ta': 'Tamil',
                    'hi': 'Hindi',
                    'te': 'Telugu',
                ...
                detected_lang_code = language if language != 'auto' else detect_text_language(text)
                detected_lang_display = LANG_MAP.get(detected_lang_code, 'English')
                ...
                llm_result = llm_service.generate_with_image(
                    ...
                    target_language=detected_lang_display,
                )
                ...
                # Generate audio
                voice_filename = tts_service.synthesize(llm_result.text, language_code=detected_lang_code)
                result: Dict = {
...
def detect_text_language(text: str) -> str:
    """Detect language of a text string based on Unicode character ranges.
    Returns 'ta', 'hi', 'te', 'kn', 'ml', or 'en'.
    """
    for char in text:
        val = ord(char)
...
def _process_text_query(
    ...
    Args:
        ...
        language_code: Chosen or auto-detected language.
    ...
    # 2. Determine target language and code
    LANG_MAP = {
        'ta': 'Tamil',
        'hi': 'Hindi',
        'te': 'Telugu',
    ...
    detected_lang_code = language_code if language_code and language_code != 'auto' else detect_text_language(user_text)
    detected_lang_display = LANG_MAP.get(detected_lang_code, 'English')

    # 3. Generate LLM response (with prompt caching, forcing the correct language)
    ...
    # 5. Generate audio (using the target language code)
    voice_filename = tts_service.synthesize(llm_result.text, language_code=detected_lang_code)
