...
class AppConfig:
    ...
    ALLOWED_AUDIO_EXTENSIONS: set = {'webm', 'wav', 'mp3', 'ogg', 'm4a', 'mp4'}   # SINGLE definition
    ...
    # Text-to-Speech (Google TTS by default for robust multilingual support, can be overridden to groq)
    TTS_PROVIDER: str = os.getenv('TTS_PROVIDER', 'gtts')  # groq or gtts
    TTS_MODEL: str = os.getenv('TTS_MODEL', 'canopylabs/orpheus-v1-english')
    TTS_VOICE: str = os.getenv('VOICE', 'autumn')

    # File upload extensions – only images defined here; audio already above
    ALLOWED_IMAGE_EXTENSIONS: set = {'jpg', 'jpeg', 'png', 'gif', 'webp'}
    MAX_IMAGE_SIZE: int = 4 * 1024 * 1024  # 4 MB for base64

    # Redis (Session Memory) – env var names corrected
    REDIS_HOST: str = os.getenv('REDIS_HOST', 'localhost')
    REDIS_PORT: int = int(os.getenv('REDIS_PORT', '6379'))
    REDIS_USERNAME: Optional[str] = os.getenv('REDIS_USERNAME')     # changed from USERNAME
    REDIS_PASSWORD: Optional[str] = os.getenv('REDIS_PASSWORD')     # changed from PASSWORD
    REDIS_SSL: bool = os.getenv('REDIS_SSL', 'true').lower() == 'true'
    REDIS_SESSION_TTL: int = int(os.getenv('REDIS_SESSION_TTL', '3600'))  # 1 hour

    # Session – prefix updated to match brand
    SESSION_TYPE: str = 'redis'
    SESSION_PERMANENT: bool = False
    SESSION_USE_SIGNER: bool = True
    SESSION_KEY_PREFIX: str = 'agrinova_ai:session:'   # was 'agri_bot:session:'
